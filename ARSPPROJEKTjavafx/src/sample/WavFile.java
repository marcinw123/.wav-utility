...
public class WavFile
{
	...
	//Marcin
	//Final's for audio normalizing algorithm
	private final int MAX_VALUE_16BIT = 32767;
	private final int MIN_VALUE_16BIT = -32768;
	private final int MAX_VALUE_24BIT = 8388607;
	private final int MIN_VALUE_24BIT = -8388608;
	...
	//Marcin
	private static int findHighestPeak(String name) throws IOException, WavFileException {
		WavFile wavFile = WavFile.openWavFile(new File(name));
		wavFile.display();
		System.out.print("Finding highest peak...");

		int[] buffer = new int[100 * wavFile.getNumChannels()];
		int framesRead;
		int max = 0;
		int maxNegative = -1;
		...
		do
		{
			...
			{
				//System.out.println(buffer[s]); //PRINT EACH SAMPLE VALUE
				if (buffer[s] > 0) {
					if (buffer[s] > max){
						max = buffer[s];
					}
					...
					if (buffer[s] < maxNegative){
						maxNegative = buffer[s];
					}
				}
			}
		}
	...
	public static void normalizeVolume(String name) throws IOException, WavFileException {

		int highestPeak = WavFile.findHighestPeak(name);
		WavFile wavFile = WavFile.openWavFile(new File(name));
		System.out.print("Raising volume...");

		WavFile wavFileToWrite = WavFile.newWavFile(new File(name.substring(0, name.length() - 4).concat("_NORM.wav")), wavFile.getNumChannels(), wavFile.getNumFrames(), wavFile.getValidBits(), wavFile.getSampleRate());
		int[] buffer = new int[100 * wavFile.getNumChannels()];
		int framesRead;
		...
		do
		{
			...
			for (int s=0; s < framesRead * wavFile.getNumChannels(); s++) {
				if (wavFile.getValidBits() == 16) {
					if (highestPeak >= 0) {
						if (buffer[s] >= 0) {
							buffer[s] += ((double)buffer[s]/(double)highestPeak) * (wavFile.MAX_VALUE_16BIT - highestPeak);
						}
						else {
							buffer[s] -= ((double)buffer[s]/(double)highestPeak) * (wavFile.MIN_VALUE_16BIT + highestPeak);
						}
					}
					else {
						if (buffer[s] >= 0) {
				...
				else if (wavFile.getValidBits() == 24) {
					if (highestPeak >= 0) {
						if (buffer[s] >= 0) {
							buffer[s] += ((double)buffer[s]/(double)highestPeak) * (wavFile.MAX_VALUE_24BIT - highestPeak);
						}
						else {
							buffer[s] -= ((double)buffer[s]/(double)highestPeak) * (wavFile.MIN_VALUE_24BIT + highestPeak);
						}
					}
					else {
						if (buffer[s] >= 0) {
				...
				// program wysypie sie jak dostanie wave'a z bitrate np. 32bit float
			}
			wavFileToWrite.writeFrames(buffer, 100);
		}
		while (framesRead != 0);

		wavFile.close();
		wavFileToWrite.close();

		System.out.println("Done");
		System.out.println(name + " normalized \n");
	}

	public static void invertPolarity(String name) throws IOException, WavFileException {
		WavFile wavFile = WavFile.openWavFile(new File(name));
		System.out.print("Inverting polarity...");

		WavFile wavFileToWrite = WavFile.newWavFile(new File(name.substring(0, name.length() - 4).concat("_INV.wav")), wavFile.getNumChannels(), wavFile.getNumFrames(), wavFile.getValidBits(), wavFile.getSampleRate());
		int[] buffer = new int[100 * wavFile.getNumChannels()];
		int framesRead;
		...
		System.out.println("Done");
		System.out.println(name + " inverted polarity\n");
	}

	public static void swapStereo(String name) throws IOException, WavFileException {

		WavFile wavFile = WavFile.openWavFile(new File(name));
		System.out.print("Swapping stereo...");

		WavFile wavFileToWrite = WavFile.newWavFile(new File(name.substring(0, name.length() - 4).concat("_SWAP.wav")), wavFile.getNumChannels(), wavFile.getNumFrames(), wavFile.getValidBits(), wavFile.getSampleRate());
		int[] buffer = new int[100 * wavFile.getNumChannels()];
		int framesRead;
		int firstSample;
		...
		do {
			framesRead = wavFile.readFrames(buffer, 100);
			for (int s=0; s < framesRead * wavFile.getNumChannels(); s+=2) {
				firstSample = buffer[s];
				buffer[s] = buffer[s+1];
				buffer[s+1] = firstSample;
			}
			wavFileToWrite.writeFrames(buffer, 100);
		}
		while (framesRead != 0);
		...
		System.out.println("Done");
		System.out.println(name + " swapped stereo\n");

	}

	public static void monophonizeAudio(String name) throws IOException, WavFileException {
		WavFile wavFile = WavFile.openWavFile(new File(name));
		System.out.print("Monophonizing... ");

		WavFile wavFileToWrite = WavFile.newWavFile(new File(name.substring(0, name.length() - 4).concat("_MONO.wav")), wavFile.getNumChannels(), wavFile.getNumFrames(), wavFile.getValidBits(), wavFile.getSampleRate());
		int[] buffer = new int[100 * wavFile.getNumChannels()];
		int framesRead;
		int monoSample;
		...
		do
		{
			framesRead = wavFile.readFrames(buffer, 100);
			for (int s=0; s < framesRead * wavFile.getNumChannels(); s+=2) {
				monoSample = (buffer[s] + buffer[s+1]) / 2;
				buffer[s] = monoSample;
				buffer[s+1] = monoSample;
			}
			wavFileToWrite.writeFrames(buffer, 100);
		}
		while (framesRead != 0);
		...
		System.out.println("Done");
		System.out.println(name + " monophonized\n");
	}
